...
bool saveToFile(JsonArray& jArr, const char* cFileName) {
  ...
  if (!spifsFile) {
    DBG_OUTPUT_PORT.println(F("Failed to open things file for writing"));
    //can_access = true;
    return false;
  }
...
bool saveNodesToFile(const Node (*ptrNodes)[NODES_LEN]) {
  ...
  const size_t bufferSize = JSON_ARRAY_SIZE(NODES_LEN) + NODES_LEN*JSON_OBJECT_SIZE(NODE_JSON_SIZE) + NODES_LEN*180;
  ...
  for( int idx = 0 ; idx < NODES_LEN ; idx++ ){
    ...
    if(tmpNode.id != 0 ){
      ...
      tmpJNode["name"] = String(tmpNode.name);
...
bool saveThingsToFile(const Thing (*ptrThings)[THINGS_LEN]) {
  ...
  const size_t bufferSize = JSON_ARRAY_SIZE(THINGS_LEN) + THINGS_LEN*JSON_OBJECT_SIZE(THING_JSON_SIZE) + THINGS_LEN*180;
  DynamicJsonBuffer jsonBuffer(bufferSize);
  ...
  for( int idx = 0 ; idx < THINGS_LEN ; idx++ ){
    ...
    if(tmpThing.id != 0){
      ...
      tmpJThing["name"] = String(tmpThing.name);
...
bool saveRecipesToFile(const Recipe (*ptrRecipes)[RECIPES_LEN]) {
  ...
  const size_t bufferSize = JSON_ARRAY_SIZE(RECIPES_LEN) + RECIPES_LEN*JSON_OBJECT_SIZE(RECIPE_JSON_SIZE) + RECIPES_LEN*180;
  DynamicJsonBuffer jsonBuffer(bufferSize);
...
int addNode(Node (*ptrNodes)[NODES_LEN], long rNodeId, const char* rNodeName, const char* rNodeIp){
  for( int idx = 0 ; idx < NODES_LEN ; idx++ ){
    Node tmpNode = (*ptrNodes)[idx];
    if((*ptrNodes)[idx].id == rNodeId ){
      String(rNodeName).toCharArray((*ptrNodes)[idx].name, NAME_SIZE);
      strncpy((*ptrNodes)[idx].ip, rNodeIp, 16);
      return ++idx;
    }
    if((*ptrNodes)[idx].id == 0 ){
      (*ptrNodes)[idx].id = rNodeId;
      String(rNodeName).toCharArray((*ptrNodes)[idx].name, NAME_SIZE);
      strncpy((*ptrNodes)[idx].ip, rNodeIp, 16);
      return ++idx;
    }
...
int deserializeNodes(Node (*ptrNodes)[NODES_LEN], char* json){
  ...
  for(JsonArray::iterator it=root.begin(); it!=root.end(); ++it){
      ...
      if (jNode["id"].as<int>() != 0) {
        ...
        jNode["name"].as<String>().toCharArray((*ptrNodes)[cuntr].name,NAME_SIZE);
        strncpy((*ptrNodes)[cuntr].ip,jNode["ip"],16);
        lastINDEX++;
        cuntr++;
      }
  }
...
bool deserializeThings(Thing (*ptrThings)[THINGS_LEN], char* json){
  ...
  for(JsonArray::iterator it=root.begin(); it!=root.end(); ++it){
      ...
      if(jThing["id"].as<int>() != 0){
        ...
        jThing["name"].as<String>().toCharArray((*ptrThings)[cuntr].name, NAME_SIZE);
